...
//anon oauth token
const token = 'GGAQ2BUKIRGJMZMU55YZ';

const results = $(".js-search-results");
const CALL_TO_URL='https://www.eventbriteapi.com/v3/events/search/?token=';
let resultHeader="";
...
function getDataFromApi(query,callback) {
    ...
    let location = `${query.location}`,
        ...
        date = `${query.date}`
        resultHeader = "<p><h2> Here are your results for " + events + " in " + location.replace(', USA', '') + " for " + date.replace('_', ' ') + "</h2></p>";
    results.html("<i>Loading events, please stand by...</i>");
    $.getJSON(CALL_TO_URL + token + '&q=' + events + '&location.address=' + location +
        '&start_date.keyword=' + date + '&location.within=' + distance + '&sort_by=date' + '&expand=venue', null, callback);
